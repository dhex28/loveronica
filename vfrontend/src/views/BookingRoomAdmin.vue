...
<template>
  ...
  <button @click="approveBooking(item.id)" class="btn btn-success btn-mb">
    Approve
  </button>
              ...
              <template v-for="(header, index) in headers" v-slot:[`header.${header.value}`]="{ props }">
                <th :key="index">
                  <v-tooltip bottom>
                    ...
                    <span>{{ header.text }}</span>
                    <input type="hidden" v-model="selectedBookingId" />
                  </v-tooltip>
                </th>
              </template>
...
<template v-slot:[`item.actions`]="{ item }">
  ...
  <button @click="denyBooking(item.id)" class="btn btn-sm custom-btn">
  Deny
</button>
</template>
        ...
        <v-container>
          <v-card>
            <v-card-title class="text-h5">Denied Room Booking</v-card-title>
        ...
        <v-container>
          <v-card>
            <v-card-title class="text-h5">Cancel Room Booking</v-card-title>
        <v-data-table :headers="headerss" :items="cancelBookings" :search="search" class="elevation-1">
...
<v-dialog v-model="denyDialog" max-width="500">
  <v-card>
    <v-card-title class="text-h5">Deny Booking</v-card-title>
    <v-card-text>
      <v-form>
        <!-- Replace v-radio-group with v-text-field for deny reason -->
        <v-textarea
          v-model="denyReason"
          label="Deny Reason"
          outlined
    ...
    <v-card-actions>
      <v-btn @click="updatebooking" text>
        Submit
      </v-btn>
...
export default {
  ...
  methods: {
    async updatebooking() {
  try {
    // Use the selectedBookingId to send the deny request
    const response = await axios.post(`/api/updateBookingStatus/${this.selectedBookingId}`, {
      reason: this.denyReason,
      status: 'denied' // Update the status to 'denied'
    });

    if (response.status === 200 && response.data.message === 'Booking denied successfully') {
      // Refresh bookings after denying
      this.fetchBookingDetails();
    } else {
      console.error('Error denying booking:', response.data);
    }
  } catch (error) {
  ...
  // Close the deny dialog and reset denyReason
  this.closeDenyDialog();
    ...
    async fetchBookingDetails() {
      try {
        ...
        this.cancelBookings = response.data.filter(booking => booking.status === 'cancel');
      } catch (error) {
    ...
    // Set the selectedBookingId to the current booking ID
